...
def locate_transaction_file():
    # Let the user choose a CSV file using the file dialog
    transaction_file_name = filedialog.askopenfilename(
        title="Select the Transaction File",
        filetypes=[("CSV Files", "*.csv")]
    )
    
    if transaction_file_name:
        label_transaction_file['text'] = transaction_file_name  # Display the selected file path
        print(transaction_file_name)  # Print the selected file path
        user_path_field.set(os.path.dirname(transaction_file_name))  # Set the directory path in the entry field
    else:
        tkinter.messagebox.showwarning("No File Selected", "Please select a valid CSV file.")
...
def map(file_coa, transaction_file_name):
    ...
    chase = pd.read_csv(transaction_file_name, dtype={'Debit': str, 'Credit': str}, skiprows=1)
    
    # Adjust column names to match the provided file format
    chase.columns = ["Status", "Date", "Description", "Debit", "Credit"]
    
    # Fill missing Debit/Credit with 0 to ensure the operations are handled correctly
    chase['Debit'] = pd.to_numeric(chase['Debit'], errors='coerce').fillna(0)
    chase['Credit'] = pd.to_numeric(chase['Credit'], errors='coerce').fillna(0)
    
    # Set a default for 'KEY' column
    chase['KEY'] = 0
    ...
    for i, row in chase.iterrows():
        match = False
        if chase.loc[i, 'Description'] == 'CHECK':
            check_number = int(chase.loc[i, 'Description'])  # Assuming checks are listed in the description
            chase.loc[i, 'KEY'] = checks.get(check_number, 0)
        else:
            # Loop through the chart of accounts (COA) for matching descriptions
            ...
            if not match:
                mia += 1
                print(i, chase.loc[i, 'KEY'], chase.loc[i, 'Description'], chase.loc[i, 'Debit'], chase.loc[i, 'Credit'])
